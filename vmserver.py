...
# 给足 3 小时,和宿主侧 relay 的 VM_TUNNEL_IDLE_TIMEOUT 对齐;对端**真的**断开时 recv 会
# 返回空串正常收尾,不靠这个超时兜底,所以放大是安全的。
PROXY_IDLE_TIMEOUT = 10800
DEFAULT_TIMEOUT = 60
# 请求里没带 timeout 时的上限。这不是"命令该跑多久"的旋钮 —— 额度由宿主传进来
# (VM_CMD_TIMEOUT / VM_LONG_TIMEOUT),这里只是兜底,给得比宿主宽,免得把它的额度切掉。
# 但它不能是"永不超时":超时是回收挂死命令的**唯一**机制,一条等输入的 cat 会永久占着
# 一个线程,而调用方早就忘了它。
MAX_TIMEOUT = 10800
