...
description: 当用户要求还原、解密或分析被混淆/加密的代码时使用,例如「这段代码被加密了,帮我解开」「这个脚本到底干了什么」「分析这个混淆的 js/php/lua」。覆盖静态去混淆与 VM 内动态脱壳,面向可疑样本分析与自有代码恢复。目前只覆盖 JavaScript / PHP / Lua 三种语言 —— 其它语言的混淆代码没有现成的 reference 与 hook 手段,遇到时如实说明,别硬套。
---
